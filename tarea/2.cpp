...
int main(){
	...
	float sumper=0; 
	 ...
	 if (x1==0 && y1==0){  //SOLUCIÓN SI PRIMER PUNTO (0,0)
	 ...
	 while ( ((x2!=0) || (y2!=0)) || ((x1!=0) || (y1!=0)) ) {
	    ...
	    cin>>y2;
	    contador++;
	    sqrt(pow(x2-x1,2)+pow(y2-y1,2)); //NO AÑADÍ LA FÓRMULA COMO VARIABLE PORQUE DABA INFINITO EN PUNTO (0,0)  
	    ...
	    sumper+=sqrt(pow(x2-x1,2)+pow(y2-y1,2));
	     ...
	     if (x2==0 && y2==0){         //AÑADIDO POR PROBLEMA DE OPERADOR LOGICO EN WHILE
	 	cout<<"Cantidad de puntos ingresados: "<<contador<<endl<<"Perimetro de figura formada: "<<sumper;
	 	return 0;
	 }
	    ...
	    sumper+=sqrt(pow(x1-x2,2)+pow(y1-y2,2));
	     ...
	     if (x1==0 && y1==0){        //AÑADIDO POR PROBLEMA DE OPERADOR LOGICO EN WHILE
	 	cout<<"Cantidad de puntos ingresados: "<<contador<<endl<<"Perimetro de figura formada: "<<sumper;
	 	return 0;
	 }
